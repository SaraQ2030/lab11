...
public class PostService {
    ...
    public List<Post> searchByCategoryName(String name) {
        List<Category> categories = new ArrayList<>();
        List<Post> list_cat = new ArrayList<>();
        Category c = categoryRepository.findCategoryByName(name);
        if (c == null) {
            throw new ApiException("Not found category name");
        }
        categories.add(c);
        for (Category category : categories) {
            List<Post> list = postRepository.findAll();
            for (Post p : list) {
                if (p.getCategory_id().equals(category.getId())) {
                    list_cat.add(p);
                }
            }
